...
<div class="content1">
	...
	<c:forEach var="prodReview" items="${prodReviewList }">
		...
		<!-- 리뷰 리스트 -->
		<div class="reviewBox">
			<img src="${contextPath}/upload/${prodReview.pdReviewImgUrl }"
				alt="${prodReview.pdReviewImgUrl }" class="productImage">
			<div class="reviewContent">
				<div class="storeName">
					<a href="#">[${prodReview.storeName }]</a>
				</div>
				<div class="productName">
					<a href="#">${prodReview.productName }</a>
				</div>
				<div class="pdRating">
					<c:forEach var="i" begin="1" end="5">
						<c:choose>
							<c:when test="${i <= prodReview.pdRating}">★</c:when>
							<c:otherwise>☆</c:otherwise>
						</c:choose>
					</c:forEach>
					${prodReview.pdRating }
				</div>
				<div class="pdReviewContent">${prodReview.pdReviewContent }</div>
